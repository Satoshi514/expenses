...
<a href = " {{ route('posts.outgo_create') }}" class="btn btn-info text-white ml-10">新規登録</a>
  ...
  <tbody>
   ...
   <tr>
    <th>収入</th>
    <td class="col-xs-1">{{ $income->amount }}</td>
   </tr> 
   
   <tr>
    <th>支出</th> 
    <td class="col-xs-1 col-ma-1 col-lg-1">{{ $expense->amount }}</td>
   </tr>
 
   <tr class="primary">
    <th class="primary">収支</th>
    <td class="col-xs-1 col-ma-1 col-lg-1 table-primary">{{$draw = $income->amount}}-{{$expense->amount}}</td>
   </tr>
   </tbody>
</table>
</div>
...
$(document).ready(function() 
    ...
    <form action="get" action="">
      ...
      <thead>
        <tr>
          <th class="col-xs-1 col-ma-1 col-lg-1 width=10">カテゴリー</th>
          <th class="col-xs-1 col-ma-1 col-lg-1  width=10">項目</th>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td class="col-xs-1 col-ma-1 col-lg-1  width=10">{{ $outgo->year }}</td>
          <td class="col-xs-1 col-ma-1 col-lg-1 width=10">{{ $outgo->month }}</td>
          ...
          <td class="col-xs-1 col-ma-1 col-lg-1"><form action="{{ Route('posts.destroy',$outgo) }}" method="post">
                @csrf
                @method('DELETE')
                <button type="submit" name="delete" value="削除" class="btn btn-danger" onclick='return confirm("削除しますか？");'>削除</button>
               </form>
          </td>
        </tr>
        @endforeach
