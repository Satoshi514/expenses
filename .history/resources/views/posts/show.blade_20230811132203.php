...
<a href = " {{ route('posts.outgo_create') }}" class="btn btn-info text-white ml-10">新規登録</a>
  ...
  <tbody>
    <tr>
      <td class="col-xs-1">{{ $incomes->year }}年</td>
    </tr>
   <tr>
    <th>収入</th>
    <td class="col-xs-1">{{ $incomes->amount }}</td>
   </tr> 
   
   <tr>
    <th>支出</th> 
    <td class="col-xs-1 col-ma-1 col-lg-1">{{ $expenses->amount }}</td>
   </tr>
 
   <tr class="primary">
    <th class="primary">収支</th>
    <td class="col-xs-1 col-ma-1 col-lg-1 table-primary">{{$income->amount}}-{{$expense->amount}}</td>
   </tr>
   </tbody>
  @endforeach
@endforeach
